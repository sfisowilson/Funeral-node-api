...
export const recalculateMy = async (req: RequestWithTenant, res: Response) => {
	try {
		const { Member, Dependent, Beneficiary, FileMetadata, RequiredDocument } = require('../models');
		const userId = req.user?.id;
		const tenantId = req.tenant?.id;
		...
		// Get member and related entities
		const member = await Member.findOne({ where: { id: userId, tenantId } });
		if (!member) {
			return res.status(404).json({ error: 'Member not found' });
		}
		...
		console.error('Error recalculating profile completion status:', err);
		return res.status(500).json({ error: 'An error occurred while recalculating profile completion status' });
	}
};
